...
<script>
function clear() {
	$(document).find('[type=text],[type=hidden]').each(function() {
		$(this).val("");
	});
}

function ItemDel() {
	if($(document).find('#obj_originalBarcode').val() != "") {
    ...
    <div class="bg1">
      <table class="table table-hover form-group">
        <tr>
          ...
          <td>
            <input class="form-control" type="text" name="obj_word" id="obj_word" onkeydown="javascript:if(event.keyCode==13){ItemSrch(); }">
          </td>
          <td>
            <input class="btn btn-primary" type="button" onclick="ItemSrch();" value="검색">
          </td>
        </tr>
        ...
        <tr>
          <td>바코드</td>
          <td>
            <input class="form-control" type="text" name="obj_barcode" id="obj_barcode">
          </td>
        </tr>
        ...
        <tr>
          <td>타입</td>
          <td>
            <select class="form-control" name="obj_type2" id="obj_type2">
        ...
        <tr>
          <td colspan="3" align=center>
          	...
          	<input class="btn btn-primary" type=button onclick="ItemUdt()" value=수정하기>
          	<input class="btn btn-primary" type=button onclick="ItemDel()" value=삭제하기>
          </td>
      </table>
    </div>
  </div>
